  ...
  <div>
    ...
    >
      <template v-slot:control>
        <div class="self-center full-width no-outline" tabindex="0">{{ transDateToChineseFormat }}</div>
      </template>
...
import { IDateStringFormat } from './type';
...
watch(
  ...
  (newValue) => {
    if (newValue !== date.value) {
      date.value = newValue ? dateUtil.formatDate(new Date(newValue), dateFormat[props.format]) : '';
    }
  },
  { immediate: true }
);
