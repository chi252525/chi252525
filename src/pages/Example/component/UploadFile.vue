<template>
  <input v-show="false" ref="uploadFile" type="file" @change="handleFileUpload" />
  ...
  <div class="upload" @click="uploadImage">
    ...
    <div v-else class="column items-center justify-center">
      ...
      <span class="textSize-16 text-primary">選擇 LOGO並預覽</span>
    </div>
  </div>
</template>
...
// 依照寬高調整圖片大小
const resizeImage = (img: HTMLImageElement, maxSize: number): { width: number; height: number } => {
...
// 壓縮圖片
const compressImage = (canvas: HTMLCanvasElement, imageQuality: number, maxSize: number): string => {
